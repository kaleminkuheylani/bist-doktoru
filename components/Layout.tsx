...
import {
  BarChart2, Bitcoin, TrendingUp, Briefcase, Info, Home,
  Menu, X, Activity, ChevronUp, ChevronDown,
} from "lucide-react";
...
function TickerStrip() {
  ...
  if (items.length === 0) {
    return (
      <div className="h-[46px] border-b flex items-center px-4"
  ...
  return (
    <div className="h-[46px] border-b"
      style={{ borderColor: "oklch(0.22 0.012 250)", background: "oklch(0.10 0.015 250)", overflow: "hidden", cursor: "grab" }}>
          ...
          <div key={i} className="ticker-item text-xs">
            ...
            {item.change && (
              <span className="flex items-center gap-0.5 font-mono text-xs"
                style={{ fontFamily: "'JetBrains Mono', monospace", color: item.up ? "oklch(0.70 0.18 160)" : "oklch(0.60 0.22 25)" }}>
...
function Sidebar({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 bottom-0 z-50 w-64 flex flex-col transition-transform duration-300 lg:translate-x-0 lg:z-auto ${isOpen ? "translate-x-0" : "-translate-x-full"}`}
        style={{ background: "oklch(0.10 0.015 250)", borderRight: "1px solid oklch(0.20 0.012 250)" }}>
        <div className="flex items-center justify-between px-4 py-4 border-b" style={{ borderColor: "oklch(0.20 0.012 250)" }}>
          <Link href="/" className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg flex items-center justify-center"
              style={{ background: "linear-gradient(135deg, oklch(0.65 0.20 220), oklch(0.75 0.18 195))", boxShadow: "0 0 16px oklch(0.65 0.20 220 / 0.4)" }}>
              <Activity className="w-5 h-5 text-white" />
            </div>
            <div>
              <div className="font-bold text-sm leading-tight" style={{ fontFamily: "'Space Grotesk', sans-serif", color: "oklch(0.95 0.005 250)" }}>BIST</div>
              <div className="font-bold text-sm leading-tight" style={{ fontFamily: "'Space Grotesk', sans-serif", color: "oklch(0.65 0.20 220)" }}>DOKTORU</div>
            </div>
          </Link>
        ...
        <div className="px-4 py-3 border-b" style={{ borderColor: "oklch(0.20 0.012 250)" }}>
          <div className="flex items-center gap-2 text-xs">
            <div className="w-2 h-2 rounded-full"
              style={{ background: marketOpen ? "oklch(0.70 0.18 160)" : "oklch(0.60 0.22 25)", animation: marketOpen ? "pulse 2s infinite" : "none" }} />
            <span style={{ color: marketOpen ? "oklch(0.70 0.18 160)" : "oklch(0.60 0.22 25)", fontFamily: "'JetBrains Mono', monospace" }}>
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "oklch(0.08 0.015 250)" }}>
      ...
      <div className="fixed top-[46px] left-0 right-0 z-30 flex items-center gap-3 px-4 py-3 lg:hidden"
        style={{ background: "oklch(0.10 0.015 250)", borderBottom: "1px solid oklch(0.20 0.012 250)" }}>
      ...
      <main className="flex-1 lg:ml-64 mt-[46px] lg:mt-[46px]" style={{ paddingTop: "2.5rem" }}>
