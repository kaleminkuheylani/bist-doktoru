...
export default function MarketHeatmap({ stocks, selectedSymbol, onSelect }: Props) {
  ...
  const drawChart = useCallback(() => {
    ...
    const layout = d3.treemap<any>()
      .size([size.w, size.h])
      .paddingOuter(4).paddingTop(20).paddingInner(1);
    ...
    svg.selectAll<SVGTextElement, d3.HierarchyRectangularNode<any>>(".slabel")
      ...
      .attr("font-weight", "700").attr("letter-spacing", "0.07em").attr("fill", "#6b7280")
    ...
    const cell = svg.selectAll<SVGGElement, typeof leaves[number]>(".cell")
      ...
      .on("mouseenter", function (event: MouseEvent, d) {
        ...
        d3.select(this).select("rect").attr("stroke", "#f9fafb").attr("stroke-width", 1.5);
      })
    ...
    cell.append("rect")
      ...
      .attr("fill", (d) => rateColor(d.data.rate)).attr("rx", 2)
    ...
    cell.filter((d) => d.x1 - d.x0 > 28 && d.y1 - d.y0 > 18)
      ...
      .attr("y", (d) => { const h = d.y1 - d.y0; return (d.y0 + d.y1) / 2 + (h > 32 ? -6 : 0); })
      .attr("text-anchor", "middle")
      .attr("font-size", (d) => (d.x1 - d.x0 > 60 ? "11px" : "9px"))
      .attr("font-family", "'Space Grotesk', sans-serif").attr("font-weight", "700")
      .attr("fill", (d) => textColor(d.data.rate)).text((d) => d.data.code);

    cell.filter((d) => d.x1 - d.x0 > 36 && d.y1 - d.y0 > 32)
      .append("text")
      .attr("x", (d) => (d.x0 + d.x1) / 2).attr("y", (d) => (d.y0 + d.y1) / 2 + 8)
    ...
    <div ref={wrapRef} style={{ width: "100%", height: "100%", position: "relative" }}>
      ...
      {tooltip && (
        <div style={{
          ...
          pointerEvents: "none", background: "oklch(0.11 0.015 250)",
          border: "1px solid oklch(0.25 0.012 250)", borderRadius: "10px",
          padding: "10px 14px", minWidth: "170px", boxShadow: "0 8px 32px oklch(0 0 0 / 0.5)",
        }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "6px 12px" }}>
            {[
              ...
              { label: "Değişim", value: `${tooltip.stock.rate >= 0 ? "+" : ""}${tooltip.stock.rate.toFixed(2)}%`, color: tooltip.stock.rate > 0 ? "#4ade80" : tooltip.stock.rate < 0 ? "#f87171" : "#9ca3af" },
      ...
      <div style={{ position: "absolute", bottom: 8, right: 10, display: "flex", alignItems: "center", gap: 4, fontSize: "9px", fontFamily: "'Space Grotesk', sans-serif", color: "#6b7280" }}>
        <span>−5%</span>
